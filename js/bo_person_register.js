...
$(document).ready(function() {
    ...
    showdata();
    $("ul li#user_info").addClass('active');
    //==================================================== See Detail User ===========================================================
$(document).on('click', 'tbody tr #btndetail', function() {

        $.post("php/moneymanagement.php", {
        DetailUser : {
            userId : $(this).parent().parent().children().html()
        } 
    }, function(data){
           if(data.length !=0)        
        {           
            if (jex.isNull(show_spage.tbl)) {
                ...
                show_spage.tbl.setAll(data);
        }
        else{
            if (jex.isNull(show_spage.tbl)) {
                show_spage.tbl = jex.plugin.newInstance("JEX_TBL","#TBL_DCONTENTS");
            }
            $("#TBL_DCONTENTS tbody").html("<tr><td colspan='8' style='text-align:center;'><div> No Data</div> </td></tr>");
        }
    });
});
        /* 
    ...
    $(document).on('blur', '#txtPasswd', function() {
    ...
    $(document).on('blur', '#txtConfirmpasswd', function() {
        if (($(this).val() == "") || ($(this).val() != $("#txtPasswd").val()))
            $(this).addClass('input_red');
        else
            $(this).removeClass('input_red');
    });
    ...
    $(document).on('click', '#btnsave', function(e) {      
        ...
        if ($("#txtPasswd").val().length == 0) {
            $("#txtPasswd").addClass("input_red");
            alert("សូមបញ្ចូល" + " " + $("#txtPasswd").attr("placeholder"));
            return;
        } else if ($("#txtPasswd").val().length < 6) {
            $("#txtPasswd").addClass("input_red");
            alert("ការបញ្ចូល" + " " + $("#txtPasswd").attr("placeholder") + " មិនត្រឹមត្រូវ។");
            return;
        }
        if ($("#txtPasswd").val() != $("#txtConfirmpasswd").val()) {
            $("#txtConfirmpasswd").addClass('input_red');
            alert("ការបញ្ចូល" + " " + $("#txtConfirmpasswd").attr("placeholder") + " មិនត្រឹមត្រូវ។");
            return false;
        }
    ...
    $(document).on('click', '#TBL_CONTENTS tr td:not(:last-child)', function() {
        ...
        $.post("php/moneymanagement.php", {
			...
			$("#txtPasswd").val(data[0].user_passwd);
			$("#txtConfirmpasswd").val(data[0].user_passwd);
			$("#txtPhone").val(data[0].user_phone);
			$("#txtAddress").val(data[0].user_address);
			$("#txtIdcard").val(data[0].user_id_card);
            $("#PHOTO_IMG").attr('src',data[0].user_photo);
			if(data[0].user_sex == "ប្រុស")
