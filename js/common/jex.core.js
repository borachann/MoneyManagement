...
_jex.prototype.isNull				= function(dat)			{ return dat==undefined||typeof(dat)==undefined||dat==null||$.trim(dat)==""; };
...
_jex.prototype.getAllData		= function(selector,formatter){ return $(selector).getAllData(formatter);	 };
			...
			if (exec.indexOf("@") > -1) {
				...
				if (execs.length > 1) {
					if (execs[0] == "SVC") {
						...
						// 20130513 , execute-target에 자기 자신을 지정할 수 있도록
						//var tar		= $(execs[1]);
...
//								}
								...
								/* params 가 있는 경우의 함수 처리가 가능하도록 수정 TODO */
								var param = "";
		...
		switch (tag.toLowerCase()) {
        	case "input":
        		if( type=="radio" ){
        			...
        			rslt = $("input[name='"+name+"']:checked").val();
        		} else {
        			rslt = $(this).val();
        		}
        	break;
...
$.fn.getAllData = function(_formatter) {
    ...
    $.fn.getAll = function(_formatter,useJexObj,fnName) {
    	...
    	if (useJexObj && id_this && jexObj001 && jexObj001.length > 0) {
            var checkUsePlugin = false;
			for (var ii=0; ii<jexObj001.length; ii++) {
				if (typeof(jexObj001[ii][fnName]) == "function"){
                    rslt = jexObj001[ii][fnName]();
                    checkUsePlugin = checkUsePlugin || true;
                }
			}
...
var _jexAjaxUtil = function(svc,ext,url) {
	...
	this.errorTrx		= true;
	this.error		= false;
	this.errFn		= function(xhr, textStatus, errorThrown) {  if (this.jm) this.jm.stop(); /*alert(_jex.getInstance().isNull(xhr.statusText)?"":(xhr.statusText+" :: ")+_jex.getInstance().toStr(errorThrown));*/};
	this.cache		= false;
	this.fn			= function() {};
	this.input		= {};
	this.option		= {};
...
_JexJobManager = Class.extend({
	init : function() {
        ...
        if (_this.runFin != (_this.jobIdx-1)) {
